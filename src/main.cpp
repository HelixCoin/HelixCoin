...
// Copyright (c) 2014 The Helixcoin developers
...
uint256 hashGenesisBlock("0x000000a58569cf7d8a52e0a4562cb1d66bd1f6fe2aea9fd607a04444f63e6f9e");
static const unsigned int timeGenesisBlock = 1393384745;
static const unsigned int nNonceGenesisBlock = 46534740;
...
const string strMessageMagic = "Helixcoin Signed Message:\n";
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 4500 HXC if made of 50 HXC inputs)
            if (nBytes < 10000)
                nMinFee = 0;
        }
        else
...
static const int64 nGenesisBlockRewardCoin = 718 * COIN; // SACRIFICE TO THE HELIX FOSSIL
static const int64 nInitialReward = 200000 * COIN; // KEEP UP SERVERS AND NODES
static const int64 nBlockRewardStartCoin = 151 * COIN;
...
int64 static GetBlockValue(int nHeight, int64 nFees, unsigned int nBits)
{
    ...
    if (nHeight == 1)
    {
        return nInitialReward;
    }

    int64 nSubsidy = nBlockRewardStartCoin;

    // Subsidy is cut in half every 3942000 blocks, which will occur approximately every year
    nSubsidy >>= (nHeight / 3942000);
...
//                          !((pindex->nHeight==91842 && pindex->GetBlockHash() == uint256("0x00000000000a4d0a398161ffc163c503763b1f4360639393e0e5.1.290e0caec")) ||
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "All Glory to the Helix Fossil";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x6338f02c5efbc864010d8e253a9292df12adf5fedc5f999e64a06c20f952e27c"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HelixcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HelixcoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("HelixcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("helixcoin-miner");
    ...
    try { while(true) {
        ...
        printf("Running HelixcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("HelixcoinMiner terminated\n");
        throw;
    }
}
